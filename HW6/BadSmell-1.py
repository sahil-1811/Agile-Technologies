...
def birth_before_marriage(table):  # US02: Birth Before Marriage
    valid_marriage = True
    data = []
    #bad smell code (unneccessary data bad data)
    value=0
    a={}
    ...
    for fam in fams:
        ...
        for per in person:
    #Bad smell Looping multiple times(if else)
            if fam.marriage is not None and value==0:
                if fam.marriage < per.birth and len(a)==0:
                    if wife_name == per.name:
...
def list_recent_births(arr4): #US35
    list_of_recent_births = []
    #Bad Smell adding unnecessary data
    str=""
    for per in person:
    ...
    # Bad smell (adding unnecessary conditions)
    if list_of_recent_births and len(str)==0:
        arr4.append(["US35", "List Recent Births", "", True, "\n".join(list_of_recent_births)])
    else:
        arr4.append(["US35", "List Recent Births","", True, "No recent Birth"])
    return arr4
